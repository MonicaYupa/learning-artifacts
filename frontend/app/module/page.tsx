    ...
    <ProtectedRoute>
      <div className="bg-dark flex h-screen flex-col">
        ...
        <header className="bg-dark-lighter flex h-12 items-center justify-between border-b border-neutral-700 px-4 sm:h-14 sm:px-6">
          <div className="flex items-center space-x-2 sm:space-x-3">
            <h1 className="text-base font-semibold text-neutral-100 sm:text-lg">
              Claude Learn:{' '}
              <span className="font-normal text-neutral-400">Learning Artifacts Demo</span>
            </h1>
          </div>
          <button
        ...
        <div className="bg-dark relative flex flex-1 flex-col overflow-hidden">
          {/* Welcome greeting - positioned absolutely when no messages */}
          {messages.length === 0 && (
            <div className="absolute left-0 right-0 top-[40%] flex -translate-y-1/2 transform items-center justify-center px-4">
              <div className="flex items-center space-x-1.5 sm:space-x-3">
                <div className="flex h-6 w-6 flex-shrink-0 items-center justify-center self-center sm:h-12 sm:w-12">
                  <div className="h-full w-full translate-y-1 sm:translate-y-2">
                    <RobotLogo />
                  </div>
                </div>
                <h1
                  className="text-xl font-light leading-none text-neutral-100 sm:text-5xl"
                                  ...
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div className="min-w-0 flex-1">
                                      ...
                                      <p className="text-xs text-neutral-400">
                                        {modules[message.moduleId].skill_level
                                          .charAt(0)
                                          .toUpperCase() +
                                          modules[message.moduleId].skill_level.slice(1)}{' '}
                                        • {modules[message.moduleId].exercises?.length || 0}{' '}
                                        exercises
                                      </p>
                                    </div>
                                  </div>
          ...
          {/* Chat Input - Centered when no messages, sticky at bottom otherwise */}
          <div
            className={`bg-dark absolute left-0 right-0 p-4 pb-6 sm:p-6 sm:pb-8 ${
              messages.length === 0 ? 'top-[45%] sm:top-[45%]' : 'bottom-0'
            }`}
          >
              ...
              <form onSubmit={handleSendMessage}>
                <div className="relative">
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full resize-none rounded-xl border-[0.5px] border-neutral-600 bg-chat-input px-5 py-4 pr-14 text-base text-neutral-100 placeholder-neutral-400 shadow-sm transition-all focus:border-neutral-500 focus:outline-none focus:ring-2 focus:ring-neutral-500/30 sm:px-6 sm:py-5 sm:pr-16"
                    style={{ verticalAlign: 'top' }}
                  />
                  <button
                    type="submit"
                    disabled={!inputMessage.trim() || loading}
                    className="absolute bottom-4 right-4 flex h-9 w-9 items-center justify-center rounded-lg bg-primary-500 text-white transition-all hover:bg-primary-600 active:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 sm:bottom-5 sm:right-5 sm:h-10 sm:w-10"
